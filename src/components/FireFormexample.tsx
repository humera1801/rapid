...
import { handleticketPDF } from "../ticket_list/handleticketPdF.js";
...
const Page: React.FC = () => {
    ...
    const handleGeneratePDF = () => {
        ...
        if (input) {
            html2canvas(input).then((canvas) => {
                ...
                if (canvas.height > pdf.internal.pageSize.height) {
    ...
    const handleCopy = (): void => {
        if (columnsOptions.length === 0) return;

        const header = columnsOptions.join(',') + '\n';
        const rows = records.map(record =>
            columnsOptions.map(col => {
                const cell = col; // Adjust as needed for each column
