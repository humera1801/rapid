...
import { generateDeliveryChallanPrint } from '../DelievryChalan/DeliveryChallanpdf/PrintChallan.js'
import { generateDeliveryChallanShare } from '../DelievryChalan/DeliveryChallanpdf/ShareChallan.js'
...
import { exportToChallanExcel } from './DeliveryChallanpdf/ExcelChallan';
import { exportToChallanListPDF } from './DeliveryChallanpdf/ChallanListpdf';
...
const ChallanList: React.FC = () => {
    ...
    const [visibleColumns, setVisibleColumns] = useState<string[]>([
        "Challan No", "Name", "State/City", "Address", "Quotation No","Mobile No", "Added By", "Action"
    ]);
    ...
    const fetchAllData = async () => {
        ...
        try {
            const data = await GetListData.getDeliveryChallanFilterdate(startDate, endDate);
    ...
    const handlefilterdate = async (startdate: string, enddate: string) => {
        try {
            ...
            const response = await GetListData.getDeliveryChallanFilterdate(startdate, enddate);
    ...
    const handleClearFilter = async () => {
        ...
        try {
            const data = await GetListData.getDeliveryChallanFilterdate();
            setRecords(data.data);
    ...
    const handleTodayList = async () => {
        ...
        try {
            const data = await GetListData.getDeliveryChallanFilterdate(today);
            setRecords(data.data);
    ...
    useEffect(() => {
        const newColumns = [
            visibleColumns.includes('Challan No') && {
                name: "Challan No",
                selector: (row: User) => row.fedc_challan_no,
                sortable: true,
                cell: (row: any) => (
                    <Link
                    href={`ViewChalan?id=${row.q_quotation_no}`}
                        style={{
                            textDecoration: 'none',
                            color: 'inherit',
            ...
            visibleColumns.includes('Action') && {
                ...
                cell: (row: User) => (
                    <div className='action-buttons'>
                        ...
                        {userRoles.includes('fireDeliveryChallan_view') && (
                            <Link href={`ViewChalan?id=${row.q_quotation_no}`} className="btn btn-sm btn-warning" >
                                <FontAwesomeIcon icon={faEye} />
                            </Link>
                        )}
                        {/* {userRoles.includes('fireDeliveryChallan_update') && (
                            <Link href={`EditChallan?id=${row.fedc_id}`} className="btn btn-sm btn-primary">
                                <FontAwesomeIcon icon={faPencilSquare} />
                            </Link>
                        )}
                        {userRoles.includes('fireDeliveryChallan_delete') && (
    ...
    const handleQuotationPrint = async (q_quotation_no: string) => {
        try {
            ...
            generateDeliveryChallanPrint(getTDetail.data[0]);
        } catch (error) {
    ...
    const handleQuotationShareClick = async (q_quotation_no: string) => {
        try {
            const getTDetail = await GetListData.GetChallanBookingId(q_quotation_no);
            generateDeliveryChallanShare(getTDetail.data[0]);
        } catch (error) {
            console.error("Error fetching ticket data:", error);
        }
    };
    ...
    const handleQuotaionListGeneratePDF = () => {
        exportToChallanListPDF(records);
    };

    const handleFireExcelExport = () => {
        exportToChallanExcel(records);
    };
            ...
            <div className="container-fluid mt-3">
                <div className="card mb-3" style={{ width: 'auto' }}>
                    ...
                    <div className="card-header d-flex justify-content-between align-items-center">
                        <h4 className="mb-0">Delivery Challan List</h4>
