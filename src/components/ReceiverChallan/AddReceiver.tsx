...
import {generateReceiverChallanPDF} from "../ReceiverChallan/ReceiverPdf/CreateReciever.js"
...
const CreateChalan = () => {
    ...
    const submitFormData = async (formData: FormData, clientId: number) => {
        try {
            const response = await axios.post('http://192.168.0.106:3001/challan/add_fire_extingusher_receiver_challan_data', formData).then((res: any) => {
                console.log("form data", res.data);
                generateReceiverChallanPDF(res.data.data);
    ...
    const submitNewClientFormData = async (formData: FormData) => {
        try {
            const response = await axios.post('http://192.168.0.106:3001/challan/add_fire_extingusher_receiver_challan_data', formData);
    ...
    const handleIngredientChange = (index: number) => (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (ingredient) {
            setValue(`service_data.${index}.fercd_capacity`, ingredient.capacity[0]);
                        ...
                        <div className="row mb-3">
                            <div className="col-lg-10 col-sm-8" style={{ display: "flex", gap: "15px" }}>
                                ...
                                <div className="col-lg-3">
                                    ...
                                    <input  {...register("ferc_date")} className="form-control form-control-sm" type="date" id="bdate" placeholder="Enter Booking date" defaultValue={currentDate} min={currentDate} />
                                </div>
                                <div className="col-lg-3">
  ...
  <h6>Client Details:</h6>                            </div>
                        ...
                        <div className="row mb-3">
                            <div className="col-lg-3 col-sm-6">
                                <label className="form-label" htmlFor="Received">Received Through</label>
                                <input
