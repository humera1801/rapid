...
export const generateReceiverChallanPDF = (formData, bankDetails) => {
    try {
        ...
        const productRows = Array.isArray(formData.service_data) ? formData.service_data.map((product, index) => `
            <tr>
                <td>${index + 1}</td>
                <td>${product.feit_name} Fire Extinguisher ${product.fercd_capacity} Capacity</td>
                <td>${product.fercd_quantity}</td>
                <td></td>
            </tr>
        `).join('') : '';
        ...
        const html = `
            ...
            <body>
                ...
                <table>
                    ...
                    <tr><td>Challan Date:</td><td>${formData.ferc_created_at}</td></tr>
