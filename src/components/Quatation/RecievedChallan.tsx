...
const ReceivedChallan = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (q_id) {
                    ...
                    if (additionalResponse.data[0]?.is_receive_challan !== 1) {
                        const q_quotation_no = additionalResponse.data[0].q_quotation_no; // Ensure q_quotation_no exists
                        if (q_quotation_no) {
                            ...
                            if (response.data && response.data.length > 0) {
                                ...
                                setValue("ferc_order_no", response.data[0].po_no);
                                ...
                                setValue("ferc_whatsapp_no", response.data[0].whatsup_no || '');
                            ...
                            const serviceData: Record<string, ProductData[]> = response.data[0].service_data;

                            Object.entries(serviceData).forEach(([fest_id, items]) => {
                                const hasNewSupply = items.some((item: ProductData) => item.fest_name === "New Supply");
                                if (!hasNewSupply) {
                                    setValue(`service_data.${fest_id}`, items);
                                }
                            });

                            const fetchedVisibleFields = new Set(
                                Object.keys(serviceData)
                                    .filter(id => {
                                        const items = serviceData[id];
                                        return !items.some((item: ProductData) => item.fest_name === "New Supply");
                                    })
                                    .map(id => parseInt(id))
                            );
                    ...
                    else if (additionalResponse.data[0]?.is_receive_challan === 1) {
                        const q_quotation_no = additionalResponse.data[0].q_quotation_no; // Ensure q_quotation_no exists
                        if (q_quotation_no) {
    ...
    const { register, control, handleSubmit, formState: { errors }, watch, setValue, getValues } = useForm<FormData>({
        defaultValues: {
            ...
            q_quotation_no: fireData.q_quotation_no || '',
            firstName: '',
            address: '',
            email: '',
            gstNo: '',
    ...
    const fetchServiceList = async () => {
        try {
            ...
            const filteredServices = response.filter((service: { fest_name: string }) => service.fest_name !== "New Supply");
    ...
    const handleDriverMobileNoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value.length <= 10) {
            ...
            setValue("ferc_driver_mobile_no", value);
        }
    };
                            ...
                            <div className="row mb-3">
                                <div className="col-lg-10 col-sm-8" style={{ display: "flex", gap: "15px" }}>
                                    ...
                                    <div className="col-lg-3">
                                        ...
                                        <input  {...register("ferc_date")} defaultValue={currentDate} className="form-control form-control-sm" type="date" />
                                    </div>
                                    <div className="col-lg-3">
...
export default ReceivedChallan;
