...
const UpdateParcelPrint = (row: FormData) => {
...
const eWayBillNos = row.bill_detail.map(bill => bill.e_way_bill_no).join(', ');

  // Calculate total invoice amount and concatenate invoice numbers
  const totalInvoiceAmount = row.bill_detail.reduce((total, bill) => {
    // Ensure invoice_amount is treated as a number
    return total + Number(bill.invoice_amount);
}, 0);


const invoiceNos = row.bill_detail.map(bill => bill.invoice_no).join(', ');
...
export default UpdateParcelPrint;
